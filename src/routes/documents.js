...
router.post('/documents', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Documento criado com sucesso', document: savedDocument });
    } catch (error) {
...
router.post('/documents/validate', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!document || !document.signedBy) {
            return res.status(400).json({ error: 'Documento não assinado ou inexistente' });
        }
        ...
        const isValid = verify.verify(document.signedBy.publicKey, signature, 'hex');
